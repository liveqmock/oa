...
public class SecureTool {
    ...
    public SecureTool(String password) {
        ...
        if (digest != null)
        {
	        m_key = new CBC(digest.digest(), 0);
	        digest.reset();
        }
    }

    public String decode(String sCipherText)
    {
        int nLen = (sCipherText.length() >> 1) & ~7;

        if (nLen < ECB.BLOCKSIZE)
            return null;

        byte[] cbciv = new byte[CBC.BLOCKSIZE];
        int nNumOfBytes = binHexToBytes(sCipherText,
                cbciv,
                0,
                0,
                CBC.BLOCKSIZE);
        if (nNumOfBytes < CBC.BLOCKSIZE)
            return null;
