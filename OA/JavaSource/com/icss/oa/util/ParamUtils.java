...
public class ParamUtils {
	 ...
	 */
	public static int[] getIntParameters(
		HttpServletRequest request,
		String name,
		int defaultNum) {
	 ...
	 */
	public static long[] getLongParameters(
		HttpServletRequest request,
		String name,
		long defaultNum) {
		String[] paramValues = request.getParameterValues(name);
	...
	/**
	 * 将得到的日期格式的parameter转换为long
	 * @param request
	 * @param name
	 * @param defaultNum 默认值
	 * @param dateFormat 日期格式  例如：yyyy-mm-dd dateFormat=1
	 * @return
	 */
	public static long getDateTimeParameter(
		HttpServletRequest request,
		String name,
		long defaultNum,
		int dateFormat) {

		String date = getParameter(request, name, false);
		if (date != null) {

			long datelong = 0;

			try {
				datelong = CommUtil.getLongByTime(date, dateFormat);
			} catch (Exception ignored) {
