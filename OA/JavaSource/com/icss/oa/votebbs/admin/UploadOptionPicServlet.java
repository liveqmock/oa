...
import com.icss.oa.votebbs.handler.OptionPicBean;
 ...
 * 上传附件
 */
public class UploadOptionPicServlet extends ServletBase {
	...
	protected void performTask(
		...
		try {
			...
			try{
				...
				System.out.println("UploadOptionPicServlet  mail_url  "+url);
			}catch(Exception e){
				response.sendRedirect(
						request.getContextPath()
							+ "/votebbs/addOption.jsp?mainid="+mainid);
			}
			// 判断附件总和是否大于10M
...
//					System.out.println("++++++++++UploadOptionAttachServlet+++path1===+++++++"+path);
					...
					System.out.println("++++++++++UploadOptionPicServlet++++++++++path=======++"+path);
					...
					System.out.println("++++++++++UploadOptionPicServlet++++++++++filepath=======++"+filepath);
					
					// ----------------------把信息装入bean---------------------------
					OptionPicBean optionPicbean = new OptionPicBean();
					optionPicbean.setFileOriginName(filename);
					// System.out.println("the uploadpath is:....."+path.toString());
					optionPicbean.setUploadUrl(path.toString());
					optionPicbean.setFilesize(file.length());
					optionPicbean.setFileMimetype(
							fileutil.getMimeType(fileutil.getFileExtension(filename)));

					System.out.println("++++++++++UploadOptionPicServlet++++++++++把信息装入bean-=======++");
