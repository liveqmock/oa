 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class StatArticalByPostServlet extends ServletBase{
	...
	protected void performTask(HttpServletRequest request, HttpServletResponse response) 
		...
		try {
			...
			System.out.println("进入投票统计StatArticalBySexServlet1++++++++++++++++++++mainid"+mainid);
				while(optionitr.hasNext()){
					...
					if("单选".equals(statarticletype.toString())){
						statvo.setPoststatnum(handler.getSingleStatByPostVO(vo.getOpId()));
					}else{
						statvo.setPoststatnum(handler.getMultiStatByPostVO(vo.getOpId()));
					}
				...
				forward(request, response, "/votebbs/statOptionByPost.jsp");
