 ...
 * Window - Preferences - Java - Code Generation - Code and Comments
 */
public class SearchShortMsgServlet extends ServletBase {
	/* (non-Javadoc)
	...
	protected void performTask(
		...
		try{
			...
			ConnLog.open("SearchShortMsgServlet");
			MsgHandler handler=new MsgHandler(conn);
			List list=handler.searchShortMsg(content,startTime,endTime,searchOld);
			request.setAttribute("list",list);
			try {
				if (conn != null) conn.close();  
			} catch (SQLException sqle) {}
			this.forward(request, response,"/message/searchShortMsg.jsp?searchOld="+searchOld);
		}catch (Exception e) {
			handleError(e);
		} finally {
			try {
				if (conn != null) {
					conn.close();
					ConnLog.close("SearchShortMsgServlet");
				}
			} catch (SQLException sqle) {
			}
		}
