 ...
 * @author		wangsu
 */
public class MainDutyManageListServlet extends ServletBase implements Servlet {
	...
	*/
	public void performTask(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			ConnLog.open("MainDutyManageListServlet");
			...
			this.forward(request, response, "/zbs/zbs_duty/dutyManageList.jsp");
			...
			try {
				if (conn != null) {
					conn.close();
					ConnLog.close("MainDutyManageListServlet");
				}
			} catch (Exception e1) {
				e1.printStackTrace();
			}
