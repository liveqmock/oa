 ...
 * @author		liupei
 */
public class YjsMainDutyManageListServlet extends ServletBase implements Servlet {
	...
	*/
	public void performTask(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			this.forward(request, response, "/zbs/yjs_duty/dutyManageList.jsp");
			...
			try {
				if (conn != null) {
					conn.close();
					ConnLog.close("YjsMainDutyManageListServlet");
				}
			} catch (Exception e1) {
				e1.printStackTrace();
			}
