 ...
 * @author		liupei
 */
public class YjsMainDutyListServlet extends ServletBase implements Servlet {
	...
	*/
	public void performTask(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			ConnLog.open("YjsMainDutyListServlet");
			...
			this.forward(request, response, "/zbs/yjs_duty/dutyList.jsp");
			...
			try {
				if (conn != null) {
					conn.close();
					ConnLog.close("YjsMainDutyListServlet");
				}
			} catch (Exception e1) {
				e1.printStackTrace();
			}
