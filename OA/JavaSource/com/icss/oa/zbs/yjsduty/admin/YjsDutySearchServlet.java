 ...
 * @author liupei
 */
public class YjsDutySearchServlet extends ServletBase {
	...
	public void performTask(HttpServletRequest request,
		...
		try {
			...
			for (int i = 0; i < mainDutyList.size(); i++) {
				TbYjsWorkinfomainVO vo = (TbYjsWorkinfomainVO) mainDutyList
						.get(i);
			...
			this.forward(request, response, "/zbs/yjs_duty/dutySearch.jsp");
			...
			try {
				if (conn != null) {
					conn.close();
					ConnLog.close("YjsDutySearchServlet");
				}
			} catch (Exception e1) {
				e1.printStackTrace();
			}
