 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class ShowSelectFolderServlet extends ServletBase {
	...
	protected void performTask(
		...
		try {
			...
			ConnLog.open("ShowSelectFolderServlet");
...
//			session.setAttribute("folderId", folderId);
			if (selectFlag.equals("0"))
				this.forward(request, response, "/mail/SelectMoveFolder.jsp");
			else
				this.forward(request, response, "/mail/SelectCopyFolder.jsp");
		} catch (Exception e) {
			handleError(e);
		} finally {
			try {
				if (conn != null){
					conn.close();
					ConnLog.close("ShowSelectFolderServlet");
				}
			} catch (Exception e1) {
				e1.printStackTrace();
			}
