...
public class ShowAddrbfolderListServlet extends ServletBase {
	protected void performTask(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ConnLog.open("ShowAddrbfolderListServlet");
			...
			this.forward(request, response, "/addressbook/tree/folderList.jsp");
			...
			try {
				if (conn != null) {
					conn.close();
					ConnLog.close("ShowAddrbfolderListServlet");
				}
			} catch (Exception e1) {
				e1.printStackTrace();
			}
