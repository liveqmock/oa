...
import com.icss.oa.mail.dao.FiletransferSetDAO;
import com.icss.oa.mail.vo.DraftPersonVO;
import com.icss.oa.mail.vo.FiletransferSetVO;
		...
		try {
			...
			if (useridlist.size() > 0) {
				for (int i = 0; i < useridlist.size(); i++) {
					...
					System.out.println("person = " + person.getLoginName());
	...
	public List getAddressList(
		...
		if (address != null) {
			...
			for (int i = 0; i < address.length; i++) {
				...
				Name =
					this.getCName(Address.substring(0, Address.indexOf("@")));
				if (i != 0)
					addressBuf.append(",");
...
//			    System.out.println("wwwwwwwwwwwwwwwwwwwwwwwwwwwww originUserAddress0000000000000000000 = "+addressBuf.toString());
			}
	...
	public String encodeBase64(String str) {
		...
		codeStr = StringUtility.replace(codeStr, "+", "|");
				String subStr = codeStr.substring(codeStr.length()-2,codeStr.length());
				if("\r\n".equals(subStr)){
	...
	public String decodeBase64(String str) {
		...
		try {
			...
			decodeStr =
				new String(
					(new sun.misc.BASE64Decoder()).decodeBuffer(decodeStr));
		} catch (IOException e) {
			e.printStackTrace();
		}
		return decodeStr;
