...
public class printuseraddress {
	...
	public printuseraddress(Connection conn) {
		this.conn = conn;
	}
	...
	/*
	public Integer writeexcel(int column, int i, Integer haschildabf)
		throws RowsExceededException, WriteException {

		UserAddressHandler handler = new UserAddressHandler(conn);
		chabfList = handler.getUserInfoList();
		...
		for (j = i + 1; chabfit.hasNext(); j++) {
			...
			if (("1").equals(chabfvo.getAbfFlag())) {
				...
				if (handler.hasChildinfo(haschildabf)) {
					...
					printuseraddress childwriteexcel = new printuseraddress(conn);
					...
					haschildabf =
						childwriteexcel.writeexcel(column, j, haschildabf);
					column = childwriteexcel.getcolumn();
					j = childwriteexcel.getrow() - 1;
					System.out.println(
						"++++++out out out " + column + j + haschildabf);
				}
			} else {
				...
				if (chabcit.hasNext()) {
					...
					System.out.println(
						"+++++++this is in the while abcontent column_j+++++++"
							+ chabcvo.getAddAbfcId()
							+ column
							+ j);
					if (j > 50) {
						j = 3;
