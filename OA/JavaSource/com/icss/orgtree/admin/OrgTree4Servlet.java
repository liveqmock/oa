...
public class OrgTree4Servlet extends ServletBase {
	...
	protected void performTask(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			}else if(sname.equals("orgAccess")){
				String orgid = request.getParameter("orguuid");
				SysOrgVO topOrgVO = handler.getOrg(orgid);
				List list = handler.getOrgTreeList(orgid);
				request.setAttribute("list", list);
				request.setAttribute("topOrgVO", topOrgVO);
				this.forward(request, response, "/orgtree/orgTree5.jsp");
			}
