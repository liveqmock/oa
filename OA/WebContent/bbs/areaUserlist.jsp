...
try {
    ...
    ConnLog.open("areaUserlist.jsp");
    BBSAreaHandler handler = new BBSAreaHandler(conn);
    List alist = handler.getAreaListById(areaId);
    ...
    String typename = "";
    typename = areaVO.getAreaname()+"专区可进人员列表";
%>
              ...
              <table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
                <tr> 
                  ...
                  <%
                     ...
                     List userList = handler.getAreAccList(areaVO.getAreaid());
                  ...
                  %>
                      <tr>
                        <td align="center" colspan="2" height="22" ><input class=bdtj type="button" name="delete" value="删  除" onClick="javascript:delete1('<%=request.getContextPath()%>')">&nbsp;&nbsp;&nbsp;&nbsp;<input class=bdtj type="button" name="back" value="返  回" onClick="javascript:window.document.location.href='<%=request.getContextPath()%>/servlet/ShowSingleAreaServlet?areaId=<%=areaVO.getAreaid().toString()%>'"></td>
                      </tr>
                   <%}else{%>
                      <tr>
...
function delete1(url)
{
	...
	if(document.form1.perid == undefined){
        ...
        if(document.form1.perid.checked){
			document.form1.action=url+"/servlet/DelAreaUserServlet";
	...
	<%
		...
		e.printStackTrace();
			...
			try {
				if (conn != null){
					conn.close();
					ConnLog.close("areaUserlist.jsp");
					}
			} catch (Exception e) {
		    }
	}
