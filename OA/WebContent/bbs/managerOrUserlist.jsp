...
try {
    ...
    ConnLog.open("managerOrUserlist.jsp");
    BBSHandler handler = new BBSHandler(conn);
    List blist = handler.getBoardListById(boardId);
    Iterator BoardIterator = blist.iterator();
    ...
    String typename = "";
    if(type.equals("1")){
        typename = boardVO.getBoardname()+"板块已有板主列表";
    }else{
        if(boardVO.getPermit().equals("1")){
             typename = boardVO.getBoardname()+"板块可进人员列表";
        }else{
             typename = boardVO.getBoardname()+"板块不可进人员列表";
        }
    }
%>
            ...
            <td width="100%">
              <table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
                <tr> 
                     ...
                     if(type.equals("1")){   //为板主列表
                         ...
                         while(managerIterator.hasNext()){ 
	                         ...
	                         if(managerVO.getBoardid().equals(boardVO.getBoardid())){
	                             smanager.append(managerVO.getUserid());
	                             smanager.append(",");
	                         }
	                     }
			             ...
			             namelist = new StringTokenizer(mnames,",");
                     }else{                 //为限制人员列表
                  ...
                  %>
                      <tr>
                        <td align="center" colspan="2" height="22" ><input class=bdtj type="button" name="delete" value="删  除" onClick="javascript:delete1('<%=request.getContextPath()%>')">&nbsp;&nbsp;&nbsp;&nbsp;<input class=bdtj type="button" name="back" value="返  回" onClick="javascript:window.document.location.href='<%=request.getContextPath()%>/servlet/ShowSingleBoardServlet?boardId=<%=boardVO.getBoardid().toString()%>'"></td>
                      </tr>
                   <%}else{%>
                      <tr>
...
function delete1(url)
{
	...
	if(document.form1.perid == undefined){
        ...
        if(document.form1.perid.checked){
			document.form1.action=url+"/servlet/DelManagerOrUserServlet";
	...
	<%
		...
		e.printStackTrace();
			...
			try {
				if (conn != null){
					conn.close();
					ConnLog.close("managerOrUserlist.jsp");
					}
			} catch (Exception e) {
		    }
	}
