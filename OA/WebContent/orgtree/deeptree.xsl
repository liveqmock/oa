  ...
  <div class="clsItem" type="leaf">
    <xsl:choose>
      ...
      <xsl:choose>
        <xsl:when test="@Href">
	      <xsl:choose>
		<xsl:when test="..[@NodeXmlSrc!='/resourceone/tempxml/0/0.xml']">
		  <a>
		    <xsl:choose>
    ...
    <div class="hide" type="container"><xsl:apply-templates /></div></div>
</xsl:template>

<xsl:template match="TreeNode[* or @NodeXmlSrc]">
  <div class="clsItem" type="parent">
    <span class="clsSpace" type="img"><span class="clsCollapse">

    +</span></span><span class="clsLabel" type="label">
