...
function loadPerson(sendType){
	...
	if(selValue.length+allnum > broadcastnum){
		var diff = broadcastnum-allnum;
		var info = "群发数量不能超过"+broadcastnum+"人,已经选择了"+allnum+"人,将截取前"+diff+"人";
		var ok = confirm(info);
		if(ok){
			length = broadcastnum-allnum;
		}else{
			length = 0;
		}
	}else{
	...
	for(var i=0;i<length;i++){
		...
		if(!checksame(selValue[i]["uuid"])){
			extstr = extstr + selValue[i]["name"] + ",";
			continue;
		}
...
function checksame(uuid){
	var obj = document.getElementById("sendto");
	var obj1 = document.getElementById("sendcc");
	var obj2 = document.getElementById("sendbcc");
	var send = obj.getElementsByTagName("span");
...
function setPersonToSend(){
	...
	document.all("addPerson_oagroup").value =sendNames_oagroup;
	...
	document.all("addcc_oagroup").value = ccNames_oagroup;
	...
	document.all("addbcc_oagroup").value = bccNames_oagroup;
