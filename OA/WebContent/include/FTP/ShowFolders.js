...
function drawcell(area,ele){
	...
	if((ele.type == 0 || ele.type == 2)){
		...
		if( area == "remotearea"){
			var comparestr = ","+(ele.path).substring(0,(ele.path).length-1)+",";
			var becompare = readstr + ","
			if(adjustshow(comparestr,becompare)){
...
function logindefault(){
	...
	document.cookie="locallogin"+"="+curcurfolder+";expires="+exp.toGMTString();
	alert("已经将目录\""+ curcurfolder +"\"设置成默认登陆目录！");
}

/**
 * 取得当前默认登陆目录
 */
function getdeflogin(){
	var cook =document.cookie;
	if(cook.indexOf("locallogin")>=0){
		var cook1 = cook.substring(cook.indexOf("locallogin"));
		var value = cook1.substring(cook1.indexOf("=")+1,cook1.indexOf(";"));
		cook2=cook.substring(cook1.indexOf(";")+1);
		//alert(cook2.substring(0,cook2.indexOf(";")));
		//alert(cook1);
		return value;
	}else{
		return "root";
...
function connecttoftp(area){
	...
	if (area == "localarea") {
		...
		var becompare = writestr + ",";
		var comparestr = ","+(curremotecurdir).substring(0,(curremotecurdir).length-1)+",";
		//alert(comparestr+"\n"+becompare);
		if(!adjustshow(comparestr,becompare)){
			alert("您没有权限往该文件夹上传文件 ！");
			return;
		}
...
function singletransfer(obj,area){
	...
	if (area == "localarea") {
		...
		if (compareCurFile(area, obj.innerHTML) == true) {
			...
			if(ok){
				barobj.style.display = "block";
				document.FtpApplet.downloadFile(obj.innerHTML,curcurfolder);
				barobj.style.display = "none";
				drawelement(curcurfolder,"localarea");
			}
		}else{
