...
function telephoneVerify(field) {
	...
	return "不符合电话号码格式！标准格式为：(xxxx)xxxxxxxx或者xxxx-xxxxxxxx";
}
...
//交验是否包含html的特殊字符 &<>?
function hasUrlCharVerify(field) {
	if (!field.value.urlChar()) {
		return success;
	}
	return "不允许包URL的保留字：=、?、&、<、>" ;
}
...
function inValueVerify(field, lower, high, includeLower, includeHigher) {
	...
	if (lower >= high) {
		return "参数错误：小值(" + lower + ")必须小于大值(" + high + ")！";
	}
...
//比某个输入域的值小，并且是正整数
function lowerFieldVerifyPlusInt(field, compField, canEquals) {
	var reValue = plusIntVerify(field);
	if (reValue == success){
		return lowerFieldVerify(field, compField, canEquals);
	}else{
		return reValue;
	}
}
//比某个输入域的值大，并且是正整数
function uperFieldVerifyPlusInt(field, compField, canEquals) {
	var reValue = plusIntVerify(field);
	if (reValue == success){
		return uperFieldVerify(field, compField, canEquals);
	}else{
		return reValue;
	}
}
//在某两个值之间校验，并且是正整数
function inValueVerifyPlusInt(field, lower, high, includeLower, includeHigher) {
	var reValue = plusIntVerify(field);
	if (reValue == success){
		return inValueVerify(field, lower, high, includeLower, includeHigher)
	}else{
