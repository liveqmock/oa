...
function loadInfo(value,bz,iftb){
 ...
 // }
   ...
   if(bz==2){
	...
	if(bz==0){
   		openSelectEnterpriseInfo('',value,'');
   }
   if(bz==1){
   		openSelectEnterpriseInfo('','',value);
...
function openSelectEnterpriseInfo(qyid,qydm,qymc){
	
   var reValue = window.showModalDialog("/share/EnterpriseInfoServlet?page=1&pageSize=15&qyid="+qyid+"&qydm="+qydm+"&qymc="+qymc,'',"dialogWidth=650px;dialogHeight=550px");
   if (reValue != null){   		
   		for(var i = 0;i<reValue.length;i++){
	  		clearAll();
			if(!ifNull(jccplbName)){
				loadCplb(reValue[i]["QYID"]);
			}
			if(!ifNull(qyidName)){			
			...
			if(!ifNull(jjlxName)){
				 var _jjlxValue = reValue[i]["JJLX"];
			    ifradioOrCheckbox(jjlxName,_jjlxValue);
				document.getElementsByName(jjlxName)[0].value = reValue[i]["JJLX"];			
			}
			
			if(!ifNull(sshyName)){			
				document.getElementsByName(sshyName)[0].value = reValue[i]["SSHY"];
			}
			
			if(!ifNull(txdzName)){			
				document.getElementsByName(txdzName)[0].value = reValue[i]["TXDZ"];
			}
			
			if(!ifNull(yzbmName)){			
				document.getElementsByName(yzbmName)[0].value = reValue[i]["YZBM"];
			}
			
			if(!ifNull(fddbrName)){
				document.getElementsByName(fddbrName)[0].value = reValue[i]["FDDBR"];
			}
			
			if(!ifNull(qylxdhName)){
				document.getElementsByName(qylxdhName)[0].value = reValue[i]["QYLXDH"];
			}
			
			if(!ifNull(zdcpmcName)){
				document.getElementsByName(zdcpmcName)[0].value = reValue[i]["ZDCPMC"];
			}
			
			if(!ifNull(yyzzbhName)){
				document.getElementsByName(yyzzbhName)[0].value = reValue[i]["YYZZBH"];
			}
			
			if(!ifNull(qycyrsName)){
				document.getElementsByName(qycyrsName)[0].value = reValue[i]["QYCYRS"];
			}
			
			if(!ifNull(qyflName)){
				document.getElementsByName(qyflName)[0].value = reValue[i]["QYFL"];
			}
			
			if(!ifNull(tzjgName)){
				document.getElementsByName(tzjgName)[0].value = reValue[i]["TZJG"];
			}
			
			if(!ifNull(xzqhName)){
				document.getElementsByName(xzqhName)[0].value = reValue[i]["XZQH"];
			}
			
			if(!ifNull(jdName)){
				document.getElementsByName(jdName)[0].value = reValue[i]["JD"];
			}
			
			if(!ifNull(wdName)){
				document.getElementsByName(wdName)[0].value = reValue[i]["WD"];
			}
			
			if(!ifNull(ztName)){
				document.getElementsByName(ztName)[0].value = reValue[i]["ZT"];
			}
			if(!ifNull(shengName)){
				document.getElementsByName(shengName)[0].value = reValue[i]["SHENG"];
			}
			if(!ifNull(shiName)){
				document.getElementsByName(shiName)[0].value = reValue[i]["SHI"];
			}
			if(!ifNull(xianName)){
				document.getElementsByName(xianName)[0].value = reValue[i]["XIAN"];
			}
  		}
   }
	...
	if(size==0){
	  //alert("没有找到符合条件的企业基本信息");
	   ...
	   for(var i=0; i<oNodes.length; i++){
			...
			if(!ifNull(jjlxName)){
			    ifradioOrCheckbox(jjlxName,JJLX);
...
function openwin(oNodes) 
{
	...
	if (reValue != null){
		...
		if(!ifNull(qyidName)){			
			...
			if(!ifNull(jjlxName)){
			     var _jjlxValue = reValue["JJLX"];
			    ifradioOrCheckbox(jjlxName,_jjlxValue);
				document.getElementsByName(jjlxName)[0].value = _jjlxValue;			
			}
...
function ifradioOrCheckbox(areaName,areaValue){
	var selElements = document.getElementsByName(areaName);
		...
		if (selElements[i].tagName.toLowerCase() == "input") {
			if (selElements[i].type.toLowerCase() == "checkbox" || selElements[i].type.toLowerCase() == "radio") {
				...
				if (selElements[i].disabled || selElements[i].readOnly){
					...
					if(selElements[i].value==areaValue){
