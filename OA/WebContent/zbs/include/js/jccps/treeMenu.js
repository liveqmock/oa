		...
		for(var i=0; i<_children.length; i++)
		{
			...
			if (_child.getAttribute("hasChildren")=="1")
			{
			   // alert("传值："+_node.id);
			    ...
			    _node.innerHTML+='<input type="checkbox" class="celltxt" name="'+_child.firstChild.data+'" value="'+_node.id+'" onClick=\'convertAllModul("'+_node.id+'","'+_node.id+'", this)\' ><span style="cursor:pointer;line-height:16px;height:16px" name="treeText"  onclick="TreeMenu.getChildren(\''+_node.id+'\');treeNodeChoosed(this,\''+_idvalue+'\',\''+_child.firstChild.data+'\');">'+_child.firstChild.data+'</span>';			    
			...
			else if (_child.getAttribute("hasChildren")==0)
			{
			    ...
			    _node.innerHTML+='<input type="checkbox" class="celltxt" name="'+_child.firstChild.data+'" value="'+_node.id+'" appid=0 ><span style="cursor:pointer;line-height:16px;height:16px" name="treeText"   onclick="treeNodeChoosed(this,\''+_idvalue+'\',\''+_child.firstChild.data+'\');">'+_child.firstChild.data+'</span>';			    
			}
			//节点加入子级容器
 ...
 * 点击菜单后的动作
 */
function treeNodeChoosed(_obj,_value,_name)
{
