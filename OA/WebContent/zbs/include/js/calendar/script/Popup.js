...
function fPopUpCalendarDlg_New(ctrlobj)
{
	...
	try{
	    ...
	    retval = window.showModalDialog("/source/style/js/calendar/masterref/CalendarDlg.htm", "", "dialogWidth:197px; dialogHeight:230px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:no; directories:yes;scrollbars:no;Resizable=no; "  );
        }
...
function fPopUpCalendarDlg_NewReturn(ctrlobj)
{
	if(N){
	    showx = 220 ; // + deltaX;
	    showy = 220; // + deltaY;
...
function getDate(elName){
	...
	return fPopUpCalendarDlg_New(elName);
}
function getSubDate(elName,i){
	//fPopUpCalendarDlg(document.getElementsByName(elName)[0]);
	var index = new Number(i);
	var reDate = fPopUpCalendarDlg_NewReturn(elName);
	if (reDate!=null){
...
function selPeople(f1,f2){
  dept=window.showModalDialog("/source/style/js/Attendance/PeopleList.jsp","","dialogWidth:380px;dialogHeight:360px;center:1;status:0");
  if(dept!=null){
...
function fPopUpCalendarDlg_App()
{
	if(N){
	try{
	    window.top.captureEvents (Event.CLICK|Event.FOCUS);
	    ...
	    newWINwidth = 202;
            ...
            winPopupWindow.win = window.open("/source/style/js/calendar/masterref/CalendarDlg.htm","CalendarDialog","dependent=yes,width="+newWINwidth + ",height=182px" )
            winPopupWindow.win.focus()
            return winPopupWindow;
	}else{
	    retval = window.showModalDialog("/source/style/js/calendar/masterref/CalendarDlg.htm", "", "dialogWidth:197px; dialogHeight:210px; status:no; directories:yes;scrollbars:no;Resizable=no; "  );
        }
