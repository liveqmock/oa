...
<%	String p_num = session.getAttribute("p_num")==null?"1":(String)session.getAttribute("p_num");
 %>
<SCRIPT language=JavaScript>
function _back(){
   window.location.href = "<%=request.getContextPath()%>/servlet/SendFileSelectCommonTwoLevelGroupServlet?flag=oa&groupid=<%= request.getAttribute("groupid1")%>&_page_num=<%=p_num%>";
}
...
if(null!=parentorganization){
 	parentorgname = parentorganization.getName()+"->";
		...
		String personJUPosition = "";
		    try{
	            personJUPosition = handler.getPersonJUPositionInJsp(sysorgpersonvo.getPersonuuid());
			}
			catch(Exception e){
				e.printStackTrace() ;
			}
