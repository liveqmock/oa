...
function _goback(type){
	document.location = "<%=request.getContextPath()%>/servlet/MessageListServlet?type="+type+"&folder=<%=folder%>";
}

function _modify(){
    document.sendForm.action = "<%=request.getContextPath()%>/address/sendfile/draf/fromDraftSendModify.jsp";
    document.sendForm.submit();
}
...
%>    
<textarea name="sendHtml" style="display:none"><%=request.getAttribute("sendHtml")%></textarea><!-- 收件人 -->
<textarea name="ccHtml" style="display:none"><%=request.getAttribute("ccHtml")%></textarea><!-- 抄送 -->
<textarea name="bccHtml" style="display:none"><%=request.getAttribute("bccHtml")%></textarea><!-- 密送 --> 
<%
}
%>
                    ...
                    <td background="<%=request.getContextPath()%>/images/bg-09.jpg"><table width="100%"  border="0" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
                          ...
                          <td bgcolor="F2F9FF" class="text-01">
                          	<textarea name="contentOld" cols="70" rows="10" class=txt2 style="display:none"><%=sendFileBean.getContent()%></textarea>
                          	<textarea name="content" cols="70" rows="10" class=txt2></textarea>
                          </td>
                        </tr>
                        <tr>
                          <td height="2" background="<%=request.getContextPath()%>/images/bg-13.gif"></td>
